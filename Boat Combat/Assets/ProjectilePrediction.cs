...
namespace Com.BowenIvanov.BoatCombat {
    public class ProjectilePrediction : MonoBehaviour
    {
        ...
        public float distancePerIteration = 1;
        ...
        void projectArc()
        {
            ...
            parent = null;
            parent = Instantiate(indicator).transform;
            Vector3 pos = gameObject.transform.position;
            pos += new Vector3(0.0f, 1.0f, 0.0f);
            Vector3 cameraDirection = (Camera.main.transform.position - gameObject.transform.position).normalized;
            Vector3 projectileDirection = new Vector3(-cameraDirection.x, cameraDirection.y, -cameraDirection.z).normalized;
            ...
            {
                Debug.DrawRay(pos, projectileDirection);
                if (i >= 6)
                    Instantiate(indicator, pos, Quaternion.identity, parent);
                projectileDirection = new Vector3(projectileDirection.x, projectileDirection.y - (((i / timeIterations)) * gravity), projectileDirection.z) * distancePerIteration;
                pos += projectileDirection;
